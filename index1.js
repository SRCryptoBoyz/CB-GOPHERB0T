...
async function loadAccounts() {
  try {
    ...
    accounts = await Promise.all(mnemonics.map(async (mnemonic) => {
    ...
    if (accounts.length === 0) {
      throw new Error("No mnemonics found in pk.txt");
    }
...
async function performStake(mnemonic, amount) {
  const wallet = await DirectSecp256k1HdWallet.fromMnemonic(mnemonic, { prefix: BECH32_PREFIX });
...
async function runDailyActivity() {
  ...
  try {
    for (let accountIndex = 0; accountIndex < accounts.length && !shouldStop; accountIndex++) {
      ...
      const { mnemonic, address } = accounts[accountIndex];
      ...
      for (let stakeCount = 0; stakeCount < dailyActivityConfig.stakeRepetitions && !shouldStop; stakeCount++) {
        ...
        try {
          await performStake(mnemonic, amount);
        } catch (error) {
...
const statusBox = blessed.box({
  ...
  style: { fg: "white", bg: "default", border: { fg: "green" } },
